...
def get_standard_parser(cmdname):
    ...
    op.add_option('-w', '--write',
                  help='(optional, default="off") Allow write access to image',
...
def load_pae_address_space(filename, dtb, mode="rb"):
    try:
        if is_crash_dump(filename) == True:
            linAS = FileAddressSpace(filename,mode=mode)
            ...
            phyAS = FileAddressSpace(filename,mode=mode)
...
def load_nopae_address_space(filename, dtb, mode="rb"):
    try:
        if is_crash_dump(filename) == True:
            linAS = FileAddressSpace(filename, mode=mode)
            phyAS = WindowsCrashDumpSpace32(linAS,0,0)
...
def load_and_identify_image(op, opts, verbose=False):
    ...
    """
    if opts.write:
        from forensics.addrspace_rw import WritableFileAddressSpace
        from forensics.x86_rw import WritableMemory
        mode = "rb+"
        IA32PagedMemory.__bases__ += (WritableMemory,)
        IA32PagedMemoryPae.__bases__ += (WritableMemory,)
        FileAddressSpace.__bases__ += (WritableFileAddressSpace,)
    else:
        mode = "rb"
    ...
    if symtab == nopae_syms:
        addr_space = load_nopae_address_space(filename, dtb, mode=mode)
    elif symtab == pae_syms:
        addr_space = load_pae_address_space(filename, dtb, mode=mode)
