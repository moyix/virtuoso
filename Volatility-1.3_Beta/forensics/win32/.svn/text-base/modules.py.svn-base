...
def modules_list(addr_space, types, symbol_table):
    ...
    if not PsLoadedModuleList is None:
        (offset, tmp)  = get_obj_offset(types, ['_LDR_DATA_TABLE_ENTRY', 'InLoadOrderLinks'])

        first_module = PsLoadedModuleList - offset

        current = read_obj(addr_space, types, ['_LDR_DATA_TABLE_ENTRY', 'InLoadOrderLinks', 'Flink'],
                           first_module)
        
        this_module = current - offset
        
        next =  read_obj(addr_space, types, ['_LDR_DATA_TABLE_ENTRY', 'InLoadOrderLinks', 'Flink'],
                         this_module)        
