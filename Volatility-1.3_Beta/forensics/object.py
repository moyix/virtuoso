...
def write_value(addr_space, value_type, vaddr, value):
    """
    Write the low-level value for a built-in type. 
    """
    ...
    buf = struct.pack('='+type_unpack_char, value)
    addr_space.write(vaddr, buf)

def write_obj(addr_space, types, member_list, vaddr, value):
    """
    Write the low-level value for some complex type's member.
    The type must have members.
    """
    ...
    (offset, current_type) = get_obj_offset(types, member_list)
    return write_value(addr_space, current_type, vaddr + offset, value)

def write_obj_to_buf(data, data_types, member_list, doffset, value):
    (offset, current_type) = get_obj_offset(data_types,member_list)
    if not builtin_types.has_key(current_type):
        raise Exception('Invalid built-in type %s' % (current_type))
    voffset = doffset + offset
    type_pack_char = builtin_types[current_type][1]
    type_size        = builtin_types[current_type][0]

    if voffset+type_size > len(data):
        return
    
    packed_val = struct.pack('='+type_pack_char, value)
    return data[:voffset] + packed_val + data[voffset+type_size:]
