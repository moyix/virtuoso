...
class Object(object):
    ...
    def __setattr__(self, attr, val):
        if not self.__dict__.has_key("__initialized__"):
...
class Void(VType):
    ...
    def set(self, theObject, val):
        buf = struct.pack("=L", val)
        theObject.vm.write(theObject.offset, buf)
...
class NativeType(VType):
    ...
    def set(self, theObject, val):
        buf = struct.pack('='+self.readChar, val)
        theObject.vm.write(theObject.offset, buf)
