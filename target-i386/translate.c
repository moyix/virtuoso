...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
 ...
 reswitch:
	...
	/*
	printf ("\n\n0x6c 0x6d -------------------\n");
	target_disas(stdout, pc_start, 200, 0);
	num_in_disas++;
	printf ("---------------------------------\n\n");
	*/
	...
	*/
    ...
    case 0xe4:
    case 0xe5:
      // these two are INPUT from the port
        if ((b & 1) == 0)
            ot = OT_BYTE;
        else
            ot = dflag ? OT_LONG : OT_WORD;
	...
	// TRL Notes 08/01
	// port number is in an immediate.  copy into T0.
        gen_op_movl_T0_im(val);
        ...
        if (gen_svm_check_io(s, pc_start,
            ...
            break;
        gen_op_in[ot]();
	// this moves the value read into T1 from the port into eax. 
	// (or ax or al depending on size) 
	// which seems to mean that the result of gen_op_in[]() is to leave the
	// result in T1?  
	...
	*/
    ...
    case 0xe6:
    case 0xe7:
      // and these two are OUTPUTS to the port
                                                 ...
                                                 int search_pc)
{
    ...
    *gen_opc_ptr = INDEX_op_end;
    ...
    // TRL 0805
    // looks like this blob of code disassembles and spits out
    // incoming block of machine code to be translated, and 
    // then spits out the generated block (in terms of micro ops,
    // it seems?).
	...
	target_disas(stdout, pc_start, pc_ptr - pc_start, disas_flags);
        fprintf(stdout, "\n");
