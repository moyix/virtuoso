...
void iferret_log_socketcall(Info_flow_syscall_t*scp) {
  ...
  case 1: // socket
	...
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SOCKET,family,type,protocol);
      }
    }
    break;
  case 2: // bind
	...
	if (sap->sin_family == 1 ) { // PF_LOCAL
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_BIND_PF_LOCAL,fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	  //           fprintf(logfile,"family  1; fd %d; file %s", fd, bptr);
	}
	else if (sap->sin_family == 2 ) { // PF_INET
	  //	  char str[1024];
	  //	  sprintf(str,"%d.%d.%d.%d:%d",b4,b5,b6,b7,b2*256+b3);
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_BIND_PF_INET,fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);

	  //           fprintf(logfile,"family  2; fd %d; %d.%d.%d.%d:%d", 
	  //                   fd, b4, b5, b6, b7, b2*256+b3);
	}
	else if (sap->sin_family == 16 ) {// PF_NETLINK
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_BIND_PF_NETLINK,fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);                  
	  ...
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_BIND_PF_PACKET,fd,b0,b1,b2,b3,b4,b5,b6,b7,*sptr,pktype,halen,tempbuf[12], tempbuf[13], tempbuf[14], tempbuf[15],tempbuf[16], tempbuf[17], tempbuf[18], tempbuf[19]);
	} else {
	  //	  char str[1024];
	  ...
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_BIND_ELSE,sap->sin_family, fd,b0,b1,b2,b3,b4,b5,b6,b7);
	}
      }
    }
    break;
  case 3: // connect
	...
	if (sap->sin_family == 1) {// PF_LOCAL
	  ...
	  //	  str = bptr;
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_CONNECT_PF_LOCAL, fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	  ...
	  //                   fd, b4, b5, b6, b7, b2*256+b3);
	  //	  sprintf(str,"%d.%d.%d.%d:%d", b4, b5, b6, b7, b2*256+b3);              
	  info_flow_log_syscall_write(scp,IFLO_SYS_SOCKETCALL_CONNECT_PF_INET, fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	}
	else {
	  //           fprintf(logfile,"socket %d; family %2d;",
	  //                   fd, sap->sin_family);
	  info_flow_log_syscall_write(scp,IFLO_SYS_SOCKETCALL_CONNECT_PF_ELSE, sap->sin_family, fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	}
      }
      //   fprintf(logfile,"\n");
    }        
  ...
  case 4: // listen
    ...
    if (paddr!=-1) {
      ...
      info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_LISTEN,*((int*) tempbuf));
    }
    //      fprintf(logfile,"\n");
    break;
  case 5: // accept
    ...
    if (paddr!=-1) {
      ...
      cpu_physical_memory_read(paddr, tempbuf, nargs[EBX] ); //-> get the args
      //   fprintf(logfile,"socket %d", *(int*) tempbuf);
      info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_ACCEPT,*(int*) tempbuf); 
    }
    //      fprintf(logfile,"\n");      
    break;
  case 6: // getsockname
    ...
    if (paddr!=-1) {
      ...
      cpu_physical_memory_read(paddr, tempbuf, nargs[EBX] ); //-> get the args
      //   fprintf(logfile,"socket %d", *(int*) tempbuf);
      info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_GETSOCKNAME,*(int*) tempbuf); 
    }
    //      fprintf(logfile,"\n");      
    break;
  case 7: // getpeername
    ...
    if (paddr!=-1) {
      ...
      cpu_physical_memory_read(paddr, tempbuf, nargs[EBX] ); //-> get the args
      //   fprintf(logfile,"socket %d", *(int*) tempbuf);
      info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_GETPEERNAME,*(int*) tempbuf); 
    }
    //      fprintf(logfile,"\n");     
    break;
  case 8: 
	...
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SOCKETPAIR,domain,type,protocol,*(int*)tempbuf); 
      }
    }          
    break;    
  case 9: // send
	...
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SEND,fd,tempbuf);
      }
      //   fprintf(logfile,"\n");  
    }
    break;
  case 10: // recv
	...
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_RECV,fd,msg,len);
      }
    }
    break;
  case 11: // sendto
	...
	if (sap->sin_family == 1 ) {// PF_LOCAL
	  ...
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SENDTO_PF_LOCAL, fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	}
	else if (sap->sin_family == 2 ) {// PF_INET
	  char str[1024];
	  ...
	  sprintf(str,"%d.%d.%d.%d:%d", b4, b5, b6, b7, b2*256+b3);
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SENDTO_PF_INET, fd,b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	}
	else {
	  //fprintf(logfile,"[dest: family %2d]", sap->sin_family);
	  info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SENDTO_PF_ELSE, sap->sin_family,fd, b0,b1,b2,b3,b4,b5,b6,b7,tempbuf);
	}
	/*
	bzero(tempbuf, 120);
	paddr = cpu_get_phys_addr(env, msg);
  ...
  case 12: // recvfrom
	...
	fd = *ptr++; 
	//   fprintf(logfile,"socket %d  ", fd);
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_RECVFROM,fd);
      }
      //   fprintf(logfile,"\n");  
    }
    break;
  case 13: // shutdown
	...
	ptr = (int*) tempbuf;
	fd = *ptr++; 
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SHUTDOWN,fd,*ptr);
	/*
	  if (*ptr == 0 ) 
  ...
  case 14: // setsockopt
	...
	//            fd, level, option);
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SETSOCKOPT,fd,level,option);
      }
      //   fprintf(logfile,"\n");
    }
    break;
  case 15: // getsockopt
	...
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_GETSOCKOPT,fd,level,option);
      }
      //   fprintf(logfile,"\n");
    }
    break;
  case 16: // sendmsg
	...
	fd = *ptr++; 
	//   fprintf(logfile,"socket %d", fd);
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_SENDMSG,fd);
      }
      //   fprintf(logfile,"\n");
    }
    break;
  case 17: // recvmsg
	...
	fd = *ptr++; 
	//   fprintf(logfile,"socket %d", fd);
	info_flow_log_sys_socketcall_write(scp,IFLO_SYS_SOCKETCALL_RECVMSG,fd);
      }
      //   fprintf(logfile,"\n");
    }
    break;
