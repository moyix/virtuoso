...
target_ulong iferret_get_arg_win(int i, int is_enter) {
    if(is_enter) {
        ...
        return ECX+(sizeof(target_ulong)*(i+2));
    }
}
...
// This is the big function that knows how to write the system call params to the log
// Used for both enter and exit.
  ...
  */
  ...
  
  {
    ...
    */    
    ...
    iferret_write_syscall_params_linux(scp);
...
void iferret_log_syscall_enter_win (uint8_t is_sysenter, uint32_t eip_for_callsite) {
...

    {
        ...
        iferret_write_syscall_params_xpsp2(scp);
    }
#endif
}
...
void iferret_log_syscall_ret_win(uint8_t is_iret, uint32_t callsite_esp, uint32_t another_eip) {
    ...
    if (element.syscall.callsite_eip != -1) {
        ...
        iferret_write_syscall_params_xpsp2(scp);
        iferret_syscall_stack_delete_at_index(pid, element.index);
	//	printf ("matching sysenter found for EDX=0%x\n", EDX);
    }
    else {
        iferret_syscall_misses++;
	...
	*/
	//	iferret_syscall_stacks_print();
	//	cpu_dump_state2(env);
	printf ("no matching sysenter for sysexit. EDX=0%x\n", EDX);
    }
#endif
}
